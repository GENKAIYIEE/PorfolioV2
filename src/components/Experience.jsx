...
export default function Experience() {
  ...
  return (
    <section id="experience" className="relative py-16 md:py-20 overflow-hidden border-t border-white/5">
      <div className="max-w-4xl mx-auto px-6">
        ...
        >
          ...
          <h2 className="section-title text-white">Experience & Education</h2>
        </motion.div>
            ...
            >
              ...
              {activeTab === tab.id && (
                <motion.div
                  ...
                  className="absolute inset-0 rounded-xl bg-blue-500/10 border border-white/20"
            ...
            >
              {filtered.map((item, i) => (
                <motion.div
                  ...
                  className="group relative flex flex-col md:flex-row gap-6 md:gap-12 py-8 border-b border-white/5 last:border-0 hover:bg-white/[0.02] -mx-6 px-6 transition-colors rounded-3xl"
                >
                  {/* Period Column */}
                  <div className="md:w-1/4 shrink-0 pt-1">
                    <span className="text-sm font-medium text-white/40 font-mono tracking-wider">
                      {item.period}
                    </span>
                  </div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-4">
                      ...
                      {item.link && (
                        <a 
                          ...
                          className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-500/10 text-blue-400 text-sm font-medium hover:bg-blue-500/20 transition-colors self-start border border-blue-500/20"
                        >
                      ...
                      {item.bullets.map((bullet, bi) => {
                        ...
                        return (
                          <li key={bi} className={`text-base leading-relaxed flex items-start ${
                            isAward ? 'text-blue-300 font-medium' : 'text-white/60'
                          }`}>
                            {isAward ? (
                              <HiStar className="w-4 h-4 text-yellow-400 mr-3 mt-1.5 shrink-0" />
                            ) : (
                              <span className="text-white/20 mr-4 mt-1">—</span>
                            )}
                            {bullet}
                          </li>
                        );
